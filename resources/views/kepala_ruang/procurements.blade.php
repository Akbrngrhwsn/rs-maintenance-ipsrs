<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-2xl text-gray-800 leading-tight">{{ __('Daftar Rekomendasi Pengadaan IPS RS') }}</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-200">
                ...
                <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                    <h3 class="text-lg font-bold text-gray-800">Status Pengadaan Ruangan</h3>
                </div>
                    ...
                    <table class="min-w-full table-auto">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            @else
                                ...
                                {{-- ======================================================== --}}
                                {{-- 1. LOOPING PENGADAAN KERUSAKAN (MAINTENANCE) DARI ADMIN  --}}
                                {{-- ======================================================== --}}
                                @foreach($procurements as $proc)
                                    <tr class="hover:bg-gray-50">
                                            ...
                                            <div id="modal-{{ $proc->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center">
                                                ...
                                                <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full relative z-10 overflow-hidden transform transition-all m-4">
                                                    ...
                                                    <div class="p-6 overflow-x-auto max-h-[70vh] overflow-y-auto">
                                                        ...
                                                        <div class="mt-4">
                                                            <a href="{{ route('kepala-ruang.procurements.export.single', $proc->id) }}" class="inline-flex items-center gap-2 bg-green-600 text-white px-3 py-1 rounded-md text-sm font-semibold hover:bg-green-700">Export Pengadaan (PDF)</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <td class="px-6 py-4 align-top text-center">
                                            @php
                                                ...
                                                $statusLabel = match($proc->status) {
                                                    'submitted_to_kepala_ruang' => 'Menunggu Validasi Anda',
                                                    'submitted_to_management' => 'Diproses Management',
                                                    'submitted_to_bendahara' => 'Diproses Bendahara',
                                                    'submitted_to_director' => 'Diproses Direktur',
                                        ...
                                        <td class="px-6 py-4 align-top text-center">
                                            {{-- Jika butuh validasi Karu, tampilkan tombol ACC/Tolak --}}
                                            @if($proc->status == 'submitted_to_kepala_ruang')
                                                <div class="flex items-center justify-center gap-2">
                                                    ...
                                                    <button type="button" onclick="document.getElementById('reject-modal-{{ $proc->id }}').classList.remove('hidden')" class="inline-flex items-center px-3 py-1 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 transition">
                                                        Tolak
                                                    </button>
                                            ...
                                            @else
                                                <span class="text-gray-400 text-sm italic">Dalam Proses Lain</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                                ...
                                {{-- ======================================================== --}}
                                {{-- 2. LOOPING PENGADAAN BARANG BARU MILIK KARU INI          --}}
                                {{-- ======================================================== --}}
                                    ...
                                    @foreach($newItemRequests as $itemReq)
                                        <tr class="hover:bg-emerald-50 border-l-4 border-emerald-500 bg-emerald-50/20">
                                            <td class="px-6 py-4 align-top">
                                                ...
                                                <div class="text-[10px] font-bold text-emerald-600 tracking-wider mt-1 border border-emerald-200 bg-emerald-100 inline-block px-2 py-0.5 rounded">PENGAJUAN SAYA</div>
                                            </td>
                                                ...
                                                <div id="modal-new-{{ $itemReq->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center">
                                                    ...
                                                    <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full relative z-10 overflow-hidden transform transition-all m-4">
                                                        ...
                                                        <div class="p-6 overflow-x-auto max-h-[70vh] overflow-y-auto">
                                                            ...
                                                            <div class="mt-4">
                                                                {{-- Pastikan ini mengarah ke route export untuk Barang Baru milik Karu --}}
                                                                <a href="{{ route('new_items.export.single', $itemReq->id) }}" target="_blank" class="inline-flex items-center gap-2 bg-green-600 text-white px-3 py-1 rounded-md text-sm font-semibold hover:bg-green-700">Export Pengajuan (PDF)</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td class="px-6 py-4 align-top text-center">
                                                {{-- Karena ini pengajuan Karu sendiri, Karu hanya melihat statusnya, tidak approve diri sendiri --}}
                                                @if(in_array($itemReq->status, ['approved', 'completed']))
                                                    <span class="text-green-600 text-sm font-bold">Disetujui</span>
